...
int _tmain()
{
    ...
    SOCKADDR_IN saFrom      = {AF_INET};
    SOCKADDR_IN saLocal     = {AF_INET};
    SOCKADDR_IN saMCast     = {AF_INET};
    ...
    //如果发送端不准备接收数据,那么不必非要绑定到指定的组播端口上
    //指定0由系统任意分配一个端口号也是可以的
    saLocal.sin_addr.s_addr      = htonl(INADDR_ANY);
    saLocal.sin_port             = htons(GRS_MCAST_PORT);
    ...
    saMCast.sin_addr.s_addr =  inet_addr(GRS_MCAST_IP);
    saMCast.sin_port = htons(GRS_MCAST_PORT);

    //准备组播地址
    CopyMemory(&ipMCast.imr_interface.s_addr,&saLocal.sin_addr.s_addr,sizeof(IN_ADDR));
    ...
    for(int i = 0 ; i< 100; i++)
    {//循环100次
        GRS_PRINTF(_T("请输入要发送的内容:"))
        StringCchGets(pReadBuf,256);
        StringCchLength(pReadBuf,256,&szLen);
        //向组内发送消息
        sendto(skServer,(const char*)pReadBuf,szLen * sizeof(TCHAR),0,(SOCKADDR*)&saMCast,isaLen);
        ...
        //do 
        //{
        //    iRecv = recvfrom(skServer,(char*)pskBufTmp,iBufLen - iBufLenTmp,0,(SOCKADDR*)&saFrom,&isaLen);
        ...
        //if(SOCKET_ERROR != iRecv && 0 != iRecv)
        //{
        //    GRS_PRINTFA("接收到客户端[%s : %u]的数据:%S\n"
        //        ,inet_ntoa(saFrom.sin_addr),ntohs(saFrom.sin_port),pskBuf);
        //}
