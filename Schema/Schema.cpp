...
int _tmain()
{
	...
	ULONG					ulSchemas					= 0;
	GUID*					pSchemasGuid				= 0;
	ULONG*					pRestrictionSupport			= 0;
	VARIANT*				pVRestrict					= 0;	
	...
	hr = pIOpenRowset->QueryInterface(IID_IDBSchemaRowset,(void**)&pIDBSchemaRowset);
	GRS_COM_CHECK(hr,_T("获取失败IDBSchemaRowset接口失败，错误码：0x%08X\n"),hr);

	hr = pIDBSchemaRowset->GetSchemas(&ulSchemas,&pSchemasGuid,&pRestrictionSupport);
	GRS_COM_CHECK(hr,_T("获取数据源支持的架构类型数组失败，错误码：0x%08X\n"),hr);
    
    ULONG iResCnt = 2;

	for(i = 0; i < ulSchemas; i++)
	{
		if(pRestrictionSupport[i] >= iResCnt)
		{//拥有约束条件的架构 为了使得到的结果集尽量小 所以加了 数据库名和表名条件限制
		 //因为并不是所有的架构结果集都支持这三个条件，所以可能某些架构打不开
			pVRestrict = (VARIANT*)GRS_CALLOC(3 * sizeof(VARIANT));	
			
			pVRestrict[0].vt = VT_BSTR;
			pVRestrict[0].bstrVal = SysAllocString(_T("Study"));
            pVRestrict[1].vt = VT_BSTR;
            pVRestrict[1].bstrVal = SysAllocString(_T("dbo"));

			//pVRestrict[1].vt = VT_NULL;
			//pVRestrict[2].vt = VT_BSTR;
			//pVRestrict[2].bstrVal = SysAllocString(_T("T_State"));

			hr = pIDBSchemaRowset->GetRowset(NULL,pSchemasGuid[i]
				,iResCnt,pVRestrict,IID_IRowset,0,NULL,(IUnknown**)&pIRowset);

			SysFreeString(pVRestrict[0].bstrVal);
			SysFreeString(pVRestrict[2].bstrVal);
			GRS_SAFEFREE(pVRestrict);
		}
		else
		{
			hr = pIDBSchemaRowset->GetRowset(NULL,pSchemasGuid[i]
		...
		if(FAILED(hr))
		{
			GRS_PRINTF(_T("获取架构信息[%lu]失败，错误码：0x%08X\n"),i,hr);
		}	
...
CLEAR_UP:
	...
	GRS_SAFEFREE(pVRestrict);
	CoTaskMemFree(pRestrictionSupport);
	CoTaskMemFree(pSchemasGuid);
	GRS_SAFEFREE(pVRestrict);
	GRS_SAFERELEASE(pIRowset);
	GRS_SAFERELEASE(pIDBSchemaRowset);
