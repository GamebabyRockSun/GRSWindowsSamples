...
int _tmain()
{
    ...
    TCHAR* pSQL = _T("Select * From T_Datetime");
    ...
    for( iCol = 0; iCol < cColumns; iCol++ )
    {
        ...
        if(DBTYPE_WSTR == rgColumnInfo[iCol].wType)
        {//把结尾的空字符算进去
 ...
 //=========================================================================================================             
                ...
                case DBTYPE_DBDATE:
                    {
                        DBDATE* pDate = (DBDATE*)(pCurData + rgBindings[iCol].obValue);
                        GRS_PRINTF(_T("%4d-%02u-%02u")
                            ,pDate->year
                            ,pDate->month
                            ,pDate->day);
                    }
                ...
                case DBTYPE_DBTIMESTAMP:
                    {
                        DBTIMESTAMP* pDatestamp = (DBTIMESTAMP*)(pCurData + rgBindings[iCol].obValue);
                        GRS_PRINTF(_T("%d-%02u-%02u %02u:%02u:%02u %u")
                            ,pDatestamp->year
                            ,pDatestamp->month
                            ,pDatestamp->day
                            ,pDatestamp->hour
                ...
                case DBTYPE_BYTES:
                    {
                        BYTE* pBytes = (BYTE*)(pCurData + rgBindings[iCol].obValue);
                        for(int k=0;k < rgColumnInfo[iCol].ulColumnSize;k++)
                        {
                            GRS_PRINTF(_T("%02X"),pBytes[k]);
                        }
                    }
                    break;
                case DBTYPE_WSTR:
                    {
                        GRS_PRINTF(_T("%-30s"),(LPCWSTR)(pCurData + rgBindings[iCol].obValue));
                    }
                    break;
                default:
                    {
                        GRS_PRINTF(_T("Unknown DBTypes"));
                    }
                    break;
                }
